-- ============================================================================
-- SIFT — Migration 0029: reframe style (track vs blur-fill)
--
-- The Clip Wizard now lets you pick how each clip is framed for 9:16:
--   track → face-tracking follow-cam (default, existing behaviour)
--   blur  → widescreen video scaled down to fit, with the SAME video enlarged
--           + blurred (slightly darkened) filling the canvas behind it
--
-- clip_jobs + clips gain a reframe_style column and create_clip_job accepts it
-- (defaults keep every older call site valid).
-- ============================================================================
...
alter table public.clip_jobs
  add column if not exists reframe_style text not null default 'track',
  add constraint clip_jobs_reframe_style_check check (
    reframe_style in ('track', 'blur')
  );

-- 2) clips: same column so the gallery can show what each clip used.
alter table public.clips
  add column if not exists reframe_style text;

-- 3) create_clip_job — new p_reframe_style param with a safe default.
--
...
begin
  ...
  insert into public.clip_jobs (
    ...
    caption_style, caption_font, caption_sub, caption_theme, reframe_style, clip_count
  )
  values (
    p_org, v_me, btrim(p_source_url), btrim(p_title),
    p_caption_style, p_caption_font, p_caption_sub, p_caption_theme, p_reframe_style, p_clip_count
  )
...
grant execute on function public.create_clip_job(uuid, text, text, text, text, text, text, text, int) to authenticated;
revoke execute on function public.create_clip_job(uuid, text, text, text, text, text, text, text, int) from anon, public;
